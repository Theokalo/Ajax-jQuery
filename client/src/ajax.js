		...
		$('#todo-list').append(
			...
			<li class="list-group-item">
					<form action="/todos/${data.id}" method="POST" class="edit-item-form">
						<div class="form-group">
							<label for="${data.id}">Item Text</label>
							<input type="text" value="${data.text}" name="todo[text]" class="form-control" id="${data.id}">
						</div>
						<button class="btn btn-primary">Update Item</button>
					</form>
					...
					<div class="pull-right">
						<button class="btn btn-sm btn-warning edit-button">Edit</button>
						<form style="display: inline" method="POST" action="/todos/${data.id}" class="delete-item-form">
							<button type="submit" class="btn btn-sm btn-danger">Delete</button>
						</form>
					</div>
					<div class="clearfix"></div>
